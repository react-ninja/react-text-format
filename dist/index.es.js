import { isValidElement, cloneElement, createElement, Fragment, Component } from 'react';
...
var MatchDecorators = function () {
  function MatchDecorators() {
      ...
      lodash.map(urls, function (url, i) {
        ...
        switch (urlType) {
          ...
          case 'mailto:':
            ...
            break;
          default:
            data.shortcode = _this.createShortcode(ENTITY.URL, _this.urls.length);
    ...
    this.findCreditCards = function () {
      var ccNums = CC_REGEXES.map(function (regex) {
        return _this.content.match(regex);
      });
      ccNums = lodash.compact(ccNums);
      ccNums = lodash.uniq(ccNums[0]);
      lodash.map(ccNums, function (cc, i) {
        var shortcode = _this.createShortcode(ENTITY.CC, i);
        _this.setContent(_this.content.replace(cc, shortcode));
        _this.cc.push(cc);
      });
    };

    this.findPhoneNumbers = function () {
      var phoneNumbers = _this.content.match(PHONE_NUMBER_REGEX);
      lodash.map(phoneNumbers, function (phone, i) {
        var shortcode = _this.createShortcode(ENTITY.PHONE, i);
        _this.setContent(_this.content.replace(phone, shortcode));
        _this.phone.push(phone);
      });
    };
    ...
    this.splitMessage = function (content) {
      ...
      if (content && content.length > 0) {
        return lodash.filter(content, function (val) {
          return val && val !== '';
        });
      }
      return null;
  ...
  createClass(MatchDecorators, [{
    key: 'createShortcode',
    value: function createShortcode(entity, id) {
      return '[' + ENTITY.SHORTCODE_PREFIX + entity + ' key=' + id + ']';
    }
  }]);
  return MatchDecorators;
}();

var formatContent = function formatContent(msg) {
  var formatMsg = new MatchDecorators();
...
var ImageDecorator = function ImageDecorator(decoratedURL, key) {
  return createElement('img', { src: decoratedURL, key: key, rel: 'noopener', className: 'rtfImage' });
};
...
var ReactFormatter = function (_React$Component) {
  inherits(ReactFormatter, _React$Component);

  function ReactFormatter() {
    classCallCheck(this, ReactFormatter);
    return possibleConstructorReturn(this, (ReactFormatter.__proto__ || Object.getPrototypeOf(ReactFormatter)).apply(this, arguments));
  }

  createClass(ReactFormatter, [{
    key: 'parseString',
      ...
      if (matches && matches.content && matches.content.length > 0) {
        var elements = lodash.map(matches.content, function (content, i) {
          if (lodash.includes(content, 'SHORTCODE:')) {
            ...
            var index = parseInt(shortcode[1].replace('key=', ''));
            switch (shortcodeType) {
              case ENTITY.URL:
                var urlData = matches.urls[index];
                if (lodash.includes(_this2.props.allowedFormats, ENTITY.URL)) {
              ...
              case ENTITY.CC:
                ...
                if (lodash.includes(_this2.props.allowedFormats, ENTITY.CC)) {
                  return _this2.props.CreditCardDecorator(ccData, i);
                }
                return ccData;
              case ENTITY.PHONE:
                var phoneData = matches.phone[index];
                if (lodash.includes(_this2.props.allowedFormats, ENTITY.PHONE)) {
                  return _this2.props.PhoneDecorator(phoneData, i);
                }
                return phoneData;
              case ENTITY.EMAIL:
                var emailData = matches.email[index];
                if (lodash.includes(_this2.props.allowedFormats, ENTITY.EMAIL)) {
                  return _this2.props.EmailDecorator(emailData.url, emailData.title, i);
                }
    ...
    value: function parse(children) {
      ...
      if (typeof children === 'string') {
        ...
        return cloneElement(children, { key: key }, this.parse(children.props.children));
